Ext.define('Rally.technicalservices.FileUtilities', {
    ...
    saveCSVToFile(csv, file_name, type_object) {
        if (type_object == undefined) {
            type_object = { type: 'text/csv;charset=utf-8' };
        }
        let blob = new Blob([csv], type_object);
        saveAs(blob, file_name);
    },

    saveTextAsFile(textToWrite, fileName, options) {
        var options = Ext.merge({ type: 'text/plain' }, options);
        console.log(options);
        let textFileAsBlob = new Blob([textToWrite], options);
